...
func (m *Weapon) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.Potential != 0 {
		i = encodeVarintWeapon(dAtA, i, uint64(m.Potential))
		i--
		dAtA[i] = 0x60
	}
	if m.Penetration != 0 {
		i -= 4
		encoding_binary.LittleEndian.PutUint32(dAtA[i:], uint32(math.Float32bits(float32(m.Penetration))))
		i--
		dAtA[i] = 0x5d
	}
	if m.Precision != 0 {
...
func (m *Weapon) Size() (n int) {
	...
	if m.Potential != 0 {
		n += 1 + sovWeapon(uint64(m.Potential))
	}
	if m.Durability != 0 {
...
func (m *Weapon) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 11:
			if wireType != 5 {
				return fmt.Errorf("proto: wrong wireType = %d for field Penetration", wireType)
			}
			var v uint32
			if (iNdEx + 4) > l {
				return io.ErrUnexpectedEOF
			...
			v = uint32(encoding_binary.LittleEndian.Uint32(dAtA[iNdEx:]))
			iNdEx += 4
			m.Penetration = float32(math.Float32frombits(v))
		case 12:
			if wireType != 0 {
