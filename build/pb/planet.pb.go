...
func (m *Planet) GetPlanetAtmosphereID() uint32 {
	if m != nil {
		return m.PlanetAtmosphereID
	}
	return 0
}

func (m *Planet) GetPlanetAtmosphere() *PlanetAtmosphere {
	if m != nil {
		return m.PlanetAtmosphere
	}
	return nil
}
...
func (m *Planet) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if m.PlanetAtmosphere != nil {
		{
			size, err := m.PlanetAtmosphere.MarshalToSizedBuffer(dAtA[:i])
			if err != nil {
				return 0, err
			}
			i -= size
	...
	if m.PlanetAtmosphereID != 0 {
		i = encodeVarintPlanet(dAtA, i, uint64(m.PlanetAtmosphereID))
		i--
		dAtA[i] = 0x50
	}
...
func (m *Planet) Size() (n int) {
	...
	if m.PlanetAtmosphereID != 0 {
		n += 1 + sovPlanet(uint64(m.PlanetAtmosphereID))
	}
	if m.PlanetAtmosphere != nil {
		l = m.PlanetAtmosphere.Size()
		n += 1 + l + sovPlanet(uint64(l))
	}
	if m.PlanetMapID != 0 {
...
func (m *Planet) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 10:
			if wireType != 0 {
				return fmt.Errorf("proto: wrong wireType = %d for field PlanetAtmosphereID", wireType)
			}
			m.PlanetAtmosphereID = 0
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
					return ErrIntOverflowPlanet
				}
				...
				b := dAtA[iNdEx]
				iNdEx++
				m.PlanetAtmosphereID |= uint32(b&0x7F) << shift
				if b < 0x80 {
					break
				}
			}
		case 11:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field PlanetAtmosphere", wireType)
			}
			var msglen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			if m.PlanetAtmosphere == nil {
				m.PlanetAtmosphere = &PlanetAtmosphere{}
			}
			if err := m.PlanetAtmosphere.Unmarshal(dAtA[iNdEx:postIndex]); err != nil {
				return err
			}
			iNdEx = postIndex
		case 12:
